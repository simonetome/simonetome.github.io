async function initLoading(){
  ...
  // images of the skybox (loaded at the init function)
  var right = new Image();
  var left = new Image();
  var top = new Image();
  ...
  right.src = "textures/skybox/right.png";
  left.src = "textures/skybox/left.png";
  top.src = "textures/skybox/top.png";
  bottom.src = "textures/skybox/bottom.png";
  front.src = "textures/skybox/front.png";
  back.src = "textures/skybox/back.png";

  const skyBoxImages = [right,left,top,bottom,front,back];

  const promiseSkyBoxArray = []; 

  for(let i=0; i < skyBoxImages.length; i++){
    promiseSkyBoxArray.push( new Promise( resolve => {
      skyBoxImages[i].onload = function(e){
  ...
  await Promise.all(promiseSkyBoxArray);
  ...
  // Create the skybox 
  skyBox = new SkyBox(gl,programSB,faceInfos);
